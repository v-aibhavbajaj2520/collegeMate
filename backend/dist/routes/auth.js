...
router.post('/login', async (req, res) => {
    try {
        ...
        if (!user.isVerified)
            return res.status(400).json({ success: false, message: 'Please verify your email first. Check your inbox for the verification OTP.' });
...
router.post('/google/callback', async (req, res) => {
    try {
        ...
        if (user) {
            if (!user.googleId)
                user = await prisma.user.update({ where: { id: user.id }, data: { googleId: googleUserInfo.id, isVerified: true, updatedAt: new Date() } });
        }
        else {
...
router.post('/google/verify', async (req, res) => {
    try {
        ...
        if (user) {
            if (!user.googleId)
                user = await prisma.user.update({ where: { id: user.id }, data: { googleId: payload.sub, isVerified: true, updatedAt: new Date() } });
        }
        else {
...
router.get('/me', authenticateToken, async (req, res) => {
    try {
        const authReq = req;
        const user = await prisma.user.findUnique({ where: { id: authReq.user.userId }, select: { id: true, email: true, name: true, role: true, isVerified: true, googleId: true, createdAt: true, updatedAt: true } });
        if (!user)
...
router.put('/update-profile', authenticateToken, async (req, res) => {
    try {
        ...
        const authReq = req;
        const updatedUser = await prisma.user.update({ where: { id: authReq.user.userId }, data: { name, updatedAt: new Date() }, select: { id: true, email: true, name: true, role: true, isVerified: true } });
